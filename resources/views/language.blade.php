...
<body>
        ...
        <div class="main-content side-content pt-0">
            ...
            <div class="container-fluid">
                ...
                <div class="page-header">
                    <h1>Language Data</h1>
                    <div class="">
                        <a class="btn ripple btn-primary" style="border-radius:3px" data-target="#add_edit_languagedata"
            ...
            <!-- Form Modal -->
            <div class="modal fade" id="add_edit_languagedata">
                <div class="modal-dialog" role="document">
                    <div class="modal-content" style="border-radius:7px">
                        <div class="modal-body pd-20 pd-sm-40">
                            ...
                            <form id="languagedata" action="{{ route('Add_Edit_LanguageData') }}" method="POST"
                                enctype="text/plain">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                                <input id="language_id" name="id" value="" type="hidden">
                                <div class="form-group">
                                    <label for="">Language Name :</label>
                                    <input id="language_name" name="language_name" class="form-control"
                                        placeholder="Enter language name" type="text">
                                    <span class="text-danger language_name_error"></span>
                                </div>
                                <div class="form-group">
    ...
    <script type='text/javascript'>
        ...
        $(document).on('click', '#add_data', function() {
            $('.modal-title').html('Add Language Data');
            document.getElementById("languagedata").reset();
        })

        $(document).on('click', '.add_edit_data', function() {
            $("#languagedata").submit(function(e) {
                e.preventDefault();
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('Add_Edit_LanguageData') }}",
                    data: form.serialize(),
                    success: function(data) {
                        if (data.success) {
                            $('#add_edit_languagedata').modal('hide');
        ...
        function edit_Languagedata(editLanguageData) {

            var language_id = $(editLanguageData).data('val');
            $('#add_edit_languagedata').modal('show');
            $('.modal-title').html('Edit Language Data');

            $.ajax({
                type: "GET",
                url: 'Edit_LanguageData/' + language_id,
                success: function(response) {
                    if (response.edit_data) {
                        $('#language_name').val(response.edit_data.language_name);
        ...
        function delete_Languagedata(deleteLanguageData) {
            var language_id = $(deleteLanguageData).data('val');

            $.ajax({
                type: "POST",
                url: "{{ route('Delete_LanguageData') }}",
                data: {
                    'id': language_id
                },
                success: function(response) {
